...
class Handler:
    ...
    def set_replenishment_process(self):
        """Choosing method, getting product"""
    ...
    def set_fallback_process(self):
        """Глобальный catch-all для нераспознанных сообщений (регистрировать последним)."""
        FallbackProcess(api=self.api, router=self.router).set()
