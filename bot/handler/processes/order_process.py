...
from bot.handler.handlers import OrderHandler
...
class OrderProcess(AbstractProcess):
    ...
    def __init__(self, api, router):
        ...
        self.service = OrderHandler(api=api)
    ...
    def set(self):
        ...
        for _state in self.stateHelper.order_form_states():
            @self.router.callback_query(
                _state,
                F.data == Callbacks.ConfirmOrder.value
            )
    ...
    def __set_base_form_parts(self):
        ...
        for _state in self.stateHelper.discrete_order_form_states():
            @self.router.callback_query(
                _state,
                F.data == Callbacks.ChangeAmount.value
            )
        ...
        for _state in self.stateHelper.delivery_order_form_states():
            @self.router.callback_query(
                _state,
                F.data == Callbacks.InputAddress.value
            )
        ...
        for _state in self.stateHelper.delivery_order_form_states():
            @self.router.callback_query(
                _state,
                F.data == Callbacks.InputFullName.value
            )
        ...
        for _state in self.stateHelper.delivery_order_form_states():
            @self.router.callback_query(
                _state,
                F.data == Callbacks.InputPhoneNumber.value
            )
        ...
        for _state in self.stateHelper.delivery_order_form_states():
            @self.router.callback_query(
                _state,
                F.data == Callbacks.InputTelegram.value
            )
        ...
        for _state in self.stateHelper.delivery_order_form_states():
            @self.router.callback_query(
                _state,
                F.data == Callbacks.ChooseDeliveryType.value
            )
