...
def wallet_category_keyboard() -> ReplyKeyboardMarkup:
    keyboard_builder = keyboards.Builder(is_custom=True)

    keyboard_builder.add_row([keyboards.List.walletCategory.value[0]])
    keyboard_builder.add_row(keyboards.List.walletCategory.value[1:])
    keyboard_builder.add_row(keyboards.List.goBack.value)
    return keyboard_builder.get_custom_keyboard()
...
def ident_form_keyboard(dto: IdentDto) -> InlineKeyboardMarkup:
    keyboard_builder = keyboards.InlineBuilder(is_custom=True)

    primary_buttons = {
        keyboards.Callbacks.InputWallet.value: '✅ Номер кошелька введен' if dto.wallet else 'Ввести номер кошелька',
        keyboards.Callbacks.ChooseService.value: 'Сменить сервис'
    }
    secondary_buttons = {
        keyboards.Callbacks.ChooseDocs.value: 'Сменить тип данных',
        keyboards.Callbacks.PremiumSwitch.value: 'Премиум: ' + ('вкл' if dto.premium else 'выкл'),
    }
    confirm_order = {
...
def wallet_form_keyboard() -> InlineKeyboardMarkup:
    keyboard_builder = keyboards.InlineBuilder(is_custom=True)
    primary_buttons = {
