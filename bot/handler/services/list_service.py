...
class AdditionalInfoMethods(Enum):
    ident = '_view_docs'  # sends media group of docs images or message with text docs
    wallet = '_view_wallet_info'  # sends txt file with card info and av codes
    bundle = None
...
class ListService(AbstractService):
    ...
    async def _view_wallet_info(self, callback: CallbackQuery, state: FSMContext, bot: Bot, additional_info: dict):
        data = additional_info['data']
        filename = f"wallet_order_{additional_info['order_id']}.txt"
        path = f"{self._get_dir(chat_id=str(callback.message.chat.id))}/{filename}"
        with open(file=path, mode='w', encoding='UTF-8') as f:
            print(data, file=f)
        file = FSInputFile(path=path, filename=filename)
        answer = self.messageBuilder.wallet_additional_info()
        reply_markup = utils.done_keyboard()
    ...
    async def choose_order_type(self, message: Message, state: FSMContext):
        """Sending message to choose order list type  (ident, wallet, bundle or debit)"""
