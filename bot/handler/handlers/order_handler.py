...
from bot.handler.factories import OrderHandlerFactory
...
class OrderHandler(AbstractHandler):
    ...
    def __init__(self, api):
        super().__init__(api)
        self.handlerFactory = OrderHandlerFactory()

    async def __get_handler(self, state: FSMContext):
        ...
        return self.handlerFactory.get_order_handler(order_type) if order_type else None
    ...
    async def __call_service_method(self, method_name: str, **kwargs):
        ...
        try:
            state = kwargs['state']
            service_name = await self.__get_handler(state)
            if not service_name:
            ...
            service: BaseOrderHandler = service_name(api=self.api)
    ...
    async def step_one(self, message: Message, state: FSMContext):
        """Step of choosing type of order (service/product/product_b/product_c)"""
    ...
    async def step_two(self, message: Message, state: FSMContext):
        ...
        service_name = self.handlerFactory.get_order_handler(order_type_short)
        if service_name:
    ...
    # перелистывание страницы в выборе товара/категории (мб где-то еще будет, поэтому выношу как можно выше и обобщаю)
    ...
    async def change_amount(self, callback: CallbackQuery, state: FSMContext):
        """Stepping into changing amount of quantiable order types (product, product_b, product_c)"""
    ...
    async def set_amount(self, message: Message, state: FSMContext):
        """Setting amount of quantiable order types (product, product_b, product_c)"""
