...
from bot.handler.dto import ServiceDto


class ServiceHandler(AbstractOrderHandler):

    def __init__(self, api):
        super().__init__(api, _type='service')
    ...
    async def __get_docs_keyboard(self, chat_id):
        """Getting reply markup with docs types for service"""
    ...
    async def __update_service_form(self, form: CallbackQuery, dto: ServiceDto, state: FSMContext):
        """Updating service order cache after changing service form"""
        update_order_resp = await self.api.update_order_cache(chat_id=form.message.chat.id,
    ...
    async def step_two(self, message, state):
        ...
        get_order_cache_resp = await self.api.get_order_cache(chat_id=message.chat.id)
        inline_markup = None

        if self._check_response(response=get_order_cache_resp):
            order_cache = get_order_cache_resp['order_cache']
            dto = ServiceDto(order_cache=order_cache)
    ...
    async def input_product_number(self, callback: CallbackQuery, state: FSMContext):
        await state.update_data(order_form=callback)
        await state.set_state(states.Order.inputProductNumber)
        answer = self.messageBuilder.input_product_number()
        await callback.answer(answer, show_alert=True)

    async def set_product_number(self, message: Message, state: FSMContext):
        number = message.text.strip()
        user_data = await state.get_data()

        dto: ServiceDto = user_data['order_dto']
        dto.set_product(number=number)
        await state.update_data(order_dto=dto)
        await state.set_state(states.Order.serviceForm)

        form: CallbackQuery = user_data['order_form']
        await self.__update_service_form(form=form, dto=dto, state=state)
        await message.delete()

    async def choose_handler(self, callback: CallbackQuery, state: FSMContext):
        """Displaying reply markup of available handlers for service: service_name (price р.)"""
        await state.update_data(order_form=callback)
        service_list = await self._get_service_list(message=callback.message, state=state, force_update=True)
        answer = self.messageBuilder.choose_handler()
        reply_markup = utils.service_reply_keyboard(_type=self.type, service_list=service_list)
        await state.set_state(states.Order.setHandler)
    ...
    async def set_handler(self, message: Message, state: FSMContext):
        ...
        service_short, service_label, service_price = (chosen_handler['short'],
                                                       chosen_handler['label'],
                                                       float(chosen_handler['price']))
        dto: ServiceDto = user_data['order_dto']
        dto.set_handler(service=service_short, service_label=service_label, price=service_price)
        await state.update_data(order_dto=dto)
        await state.set_state(self._form_state())
    ...
    async def switch_priority(self, callback: CallbackQuery, state: FSMContext):
        user_data = await state.get_data()
        dto: ServiceDto = user_data['order_dto']
        dto.switch_priority(price=dto.priority_price)
        await state.update_data(order_dto=dto)
        await state.update_data(order_form=callback)
        answer = self.messageBuilder.priority_switched(status=dto.priority)
        await callback.answer(answer, show_alert=True)
        await self.__update_service_form(form=callback, dto=dto, state=state)
    ...
    async def __images_deleted(self, order_cache_id, chat_id, dto: ServiceDto):
    ...
    async def __after_image_upload(self, form: CallbackQuery, dto: ServiceDto, state: FSMContext):
        user_data = await state.get_data()
