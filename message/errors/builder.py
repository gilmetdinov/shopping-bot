...
class ErrorMessageBuilder:
    ...
    def get_message_by_code(self, code):
        ...
        # 3
        elif code == self.errorCodes.ServiceAlreadyBound.value:
        ...
        # -4
        elif code == self.errorCodes.PaymentError.value:
            return (f"Ошибка создания реквизитов для оплаты!\n"
                    f"Пожалуйста, обратитесь в нашу поддержку: "
                    f"{self.supportTg}", True)
    ...
    def get_incorrect_service_message(self):
        return (f"Выбранный Вами вариант не найден."
                f"\nЕсли Вы уверены, что выбрали корректный вариант, пожалуйста, обратитесь в нашу поддержку:"
                f"{self.supportTg}"), False
